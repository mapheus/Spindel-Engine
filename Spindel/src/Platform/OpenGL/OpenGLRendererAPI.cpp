...
namespace Spindel {
	...
	static void OpenGLLogMessage(GLenum source, GLenum type, GLuint id, GLenum severity, GLsizei length, const GLchar* message, const void* userParam)
	{
		...
		case GL_DEBUG_SEVERITY_HIGH:
			SP_CORE_ERROR("[OpenGL Debug HIGH] {0}", message);
			SP_CORE_ASSERT(false, "GL_DEBUG_SEVERITY_HIGH");
			break;
		case GL_DEBUG_SEVERITY_MEDIUM:
			SP_CORE_WARN("[OpenGL Debug MEDIUM] {0}", message);
			break;
		case GL_DEBUG_SEVERITY_LOW:
			SP_CORE_INFO("[OpenGL Debug LOW] {0}", message);
			break;
	...
	void RendererAPI::Init()
	{
		...
		SP_TRACE("RendererAPI was init");

		GLenum error = glGetError();
		while (error != GL_NO_ERROR)
		{
			SP_CORE_ERROR("OpenGL Error {0}", error);
			error = glGetError();
		}
