...
namespace Spindel {
	...
	#if UNIFORM_LOGGING
	#define SP_LOG_UNIFORM(...) SP_CORE_WARN(__VA_ARGS__)
	#else
	#define SP_LOG_UNIFORM
	#endif
	...
	void OpenGLShader::Reload(const std::string& source)
	{
		Load(source);
	}
	...
	std::unordered_map<GLenum, std::string> OpenGLShader::PreProcess(const std::string& source)
	{
		...
		while (pos != std::string::npos)
		{
			...
			SP_CORE_ASSERT(type == "vertex" || type == "fragment" || type == "pixel" || type == "compute", "Invalid shader type specified");
	...
	int32_t OpenGLShader::GetUniformLocation(const std::string& name) const
	{
		...
		if (result == -1)
			SP_CORE_WARN("Could not find uniform '{0}' in shader", name);
	...
	void OpenGLShader::CompileAndUploadShader()
	{
		...
		for (auto& kv : m_ShaderSource)
		{
			...
			if (isCompiled == GL_FALSE)
			{
				...
				SP_CORE_ERROR("Shader compilation failed ({0}):\n{1}", m_AssetPath, &infoLog[0]);
		...
		if (isLinked == GL_FALSE)
		{
			...
			SP_CORE_ERROR("Shader linking failed ({0}):\n{1}", m_AssetPath, &infoLog[0]);
	...
	void OpenGLShader::ResolveAndSetUniform(OpenGLShaderUniformDeclaration* uniform, Buffer buffer)
	{
		...
		SP_CORE_ASSERT(uniform->GetLocation() != -1, "Uniform has invalid location!");
		...
		case OpenGLShaderUniformDeclaration::Type::STRUCT:
			...
			break;
		default:
			SP_CORE_ASSERT(false, "Unknown uniform type!");
		}
	}
	...
	void OpenGLShader::UploadUniformFloat(const std::string& name, float value)
	{
		...
		if (location != -1)
			...
			SP_LOG_UNIFORM("Uniform '{0}' not found!", name);
	}
